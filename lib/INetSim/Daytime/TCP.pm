# -*- perl -*-
#
# INetSim::Daytime::TCP - A fake TCP daytime server
#
...
sub configure_hook {
    ...
    $self->{server}->{port}   = &INetSim::Config::getConfigParameter("Daytime_TCP_BindPort");  # bind to port
    ...
    $self->{servicename} = &INetSim::Config::getConfigParameter("Daytime_TCP_ServiceName");
