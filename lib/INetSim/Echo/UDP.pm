# -*- perl -*-
#
# INetSim::Echo::UDP - A fake UDP echo server
#
...
sub configure_hook {
    ...
    $self->{server}->{port}   = &INetSim::Config::getConfigParameter("Echo_UDP_BindPort");  # bind to port
...
#    $self->{server}->{udp_recv_len} = 1024;                              # default is 4096
    ...
    $self->{servicename} = &INetSim::Config::getConfigParameter("Echo_UDP_ServiceName");
...
sub process_request {
    ...
    if ($self->{server}->{numchilds} >= $self->{max_childs}) {
        ...
        $client->send($recvmsg, 0);
        $recvmsg =~ s/^[\r\n]+//g;
        $recvmsg =~ s/[\r\n]+$//g;
        if ($recvmsg ne "") {
            &INetSim::Log::SubLog("[$rhost:$rport] recv: " . $recvmsg, $self->{servicename}, $$);
            &INetSim::Log::SubLog("[$rhost:$rport] send: " . $recvmsg, $self->{servicename}, $$);
            $stat_success = 1;
        }
    }
